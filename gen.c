  ...
  if (ctype->type == CTYPE_ARRAY) {
    if (off)
      if (ctype->ptr->type == CTYPE_INT)
        emit("lea _%s+%d(%%rip), %%rax", label, off);
      else
        emit("lea %s+%d(%%rip), %%rax", label, off);
    else
      if (ctype->ptr->type == CTYPE_INT)
        emit("lea _%s(%%rip), %%rax", label);
      else
...
static void emit_gsave(char *varname, Ctype *ctype, int off) {
  ...
  if (off)
    emit("mov %%%s, _%s+%d(%%rip)", reg, varname, off);
  else
    emit("mov %%%s, _%s(%%rip)", reg, varname);
}
...
void emit_data_section(void) {
  ...
  for (Iter *i = list_iter(globalenv->vars); !iter_end(i);) {
    ...
    if (v->type == AST_STRING) {
      ...
      emit(".ascii \"%s\\0\"", quote_cstring(v->sval)); // [Fixed from] printf(".ascii \"%s\"\n", quote_cstring(p->sval));
...
static void emit_bss(Ast *v) {
  SAVE;
  emit(".lcomm _%s, %d", v->declvar->varname, v->declvar->ctype->size);
}
...
static void emit_func_prologue(Ast *func) {
  ...
  emit(".text");
  emit_label(".globl _%s", func->fname);
